namespace 本身隔離了 `Network Stack`, 這意味包含了 `interface`, `ip address`, `iptagbles`, `route` 等各式各樣跟網路有關的資源都被隔離。

接下來我們可以做一個簡單的操作來看看，操作上我們都會使用 `ip netns` 的指令來使用
...
1.  先取得該 `Container Process` 於 `Host` 上的 `PID`
2.  前往該 `PID` 於 `/proc/xxxx/ns` 底下找到所有的 `namespace`
3.  將上述發現的 `namespace` 建立連結到 `/var/run/ns`
4.  可以使用 `ip netns` 等指令來操作
...
使用 `docker` 的人一定對於 `volume mount` 這個概念不陌生，不論是 `docker volume` 更上層的抽象化或是單純運行時期掛載上去的 `docker run -v xxx:xxx` 等都能夠用來解決部分的 `Container` 內的需要的儲存問題
